...
{
    public class AcceptDeleteProductValidator : IValidator<AcceptDeleteProductDtoBL>
    {
        private readonly IDbContext context;
        private readonly IProductQueries queries;

        public AcceptDeleteProductValidator(IDbContext context, IProductQueries queries)
        {
            this.context = context;
            this.queries = queries;
        }
        public async Task<object> Validate(AcceptDeleteProductDtoBL dto)
        {
            if (dto.ProductId < 0)
