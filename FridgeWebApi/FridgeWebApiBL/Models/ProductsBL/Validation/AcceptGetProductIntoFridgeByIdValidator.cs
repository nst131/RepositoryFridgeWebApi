...
{
    public class AcceptGetProductIntoFridgeByIdValidator : IValidator<AcceptGetProductIntoFridgeByIdDtoBL>
    {
        private readonly IDbContext context;
        private readonly IProductQueries queries;

        public AcceptGetProductIntoFridgeByIdValidator(IDbContext context, IProductQueries queries)
        {
            this.context = context;
            this.queries = queries;
        }

        public async Task<object> Validate(AcceptGetProductIntoFridgeByIdDtoBL dto)
        {
            if (dto.FridgeProductId < 0)
                throw new ElementOutOfRangeException($"{dto.FridgeProductId} cannot be zero");

            var entity = await this.context.ExecuteQueryAndRead<Entity>(this.queries.QueryOnExistFridgeProduct(dto.FridgeProductId, this.context.GetDatabase));
